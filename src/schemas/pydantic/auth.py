...
class AuthUserResponse(BaseSchema):
	...
	username: str | None = Field(
		description="Telegram username (без @), может отсутствовать",
	)
	role: str | None = Field(
		description="Роль пользователя: viewer | streamer. None — роль ещё не выбрана",
	)
...
class AuthResponse(BaseSchema):
	is_new_user: bool = Field(
		description="True — пользователь зарегистрирован впервые",
	)
