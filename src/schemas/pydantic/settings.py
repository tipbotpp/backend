...
class AlertSettingsResponse(BaseSchema):
    ...
    image_enabled: bool = Field(description="Включена ли генерация изображения к донату")
...
class PassiveIncomeResponse(BaseSchema):
    enabled: bool = Field(description="Включён ли пассивный доход для зрителей")
    coins_per_interval: int = Field(description="Количество монет за один интервал")
