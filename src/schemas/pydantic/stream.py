...
class StreamStartResponse(BaseSchema):
    ...
    stream_token: str = Field(description="Уникальный токен стрима — используется для виджета и WebSocket")
    widget_url: str = Field(description="URL OBS Browser Source для подключения виджета")
...
class StreamStopResponse(BaseSchema):
    ...
    total_collected: int = Field(description="Итоговая сумма донатов за стрим в монетах")
...
class StreamStatusResponse(BaseSchema):
    ...
    session_id: int | None = Field(description="ID активной сессии, None если стрим не идёт")
    started_at: datetime | None = Field(description="Время начала активного стрима, None если не идёт")
    widget_url: str | None = Field(description="URL виджета для OBS, None если стрим не идёт")
