...
class DonationBody(BaseSchema):
    streamer_id: int = Field(gt=0, description="Telegram ID стримера-получателя")
...
class DonationCreateResponse(BaseSchema):
    ...
    status: str = Field(description="Статус доната: processing | completed | rejected")
...
class DonationHistoryItemResponse(BaseSchema):
    ...
    status: str = Field(description="Статус: processing | completed | rejected")
    audio_url: str | None = Field(description="Presigned URL на TTS-аудио (временный)")
    image_url: str | None = Field(description="Presigned URL на сгенерированное изображение (временный)")
    from_user: DonorResponse = Field(description="Отправитель доната")
    to_streamer: DonorResponse = Field(description="Получатель доната (стример)")
...
class SessionStatsResponse(BaseSchema):
    ...
    total_collected: int = Field(description="Итоговая сумма донатов за сессию в монетах")
    donations_count: int = Field(description="Количество донатов за сессию")
    top_donator: TopDonatorResponse | None = Field(description="Топ-донатер сессии, None если донатов не было")
    timeline: list[TimelineItemResponse] = Field(description="Динамика донатов по минутам")
