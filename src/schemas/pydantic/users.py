...
class UserResponse(BaseSchema):
	...
	display_name: str | None = Field(
		description="Отображаемое имя (кастомное или из Telegram)",
	)
	avatar_url: str | None = Field(description="URL аватара из Telegram")
	role: str | None = Field(
		description="Роль: viewer | streamer. None — не выбрана",
	)
...
class UserRoleResponse(BaseSchema):
	...
	balance: int = Field(
		description="Начальный баланс (100 монет для viewer, 0 для streamer)",
	)
...
class StreamerItemResponse(BaseSchema):
	...
	goal: GoalPreview | None = Field(
		description="Текущая цель сбора, если задана",
	)
...
class StreamerProfileResponse(BaseSchema):
	...
	alert_preview: AlertPreview | None = Field(
		description="Настройки стиля алерта для превью виджета",
	)
