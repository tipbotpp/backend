...
@router.patch("/me/role", response_model=users_schema.UserRoleResponse, summary="Установить роль (viewer / streamer) — только один раз")
@inject
async def set_role(
...
@router.get("/streamers", response_model=users_schema.StreamerListResponse, summary="Список стримеров с активным стримом")
@inject
    ...
    items = [
        users_schema.StreamerItemResponse(
            ...
            goal=(
                ...
                if s.telegram_id in goal_map else None
            ),
        )
        for s in streamers
    ]
...
@router.get("/{user_id}", response_model=users_schema.StreamerProfileResponse, summary="Публичный профиль стримера")
@inject
