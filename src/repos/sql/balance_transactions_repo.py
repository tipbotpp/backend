...
async def get_by_ids(
	session: AsyncSession,
	ids: list[int],
) -> list[BalanceTransactionDTO]:
	result = await session.execute(
		select(BalanceTransactions).where(BalanceTransactions.id.in_(ids)),
	)
	return [
		map_model(row, BalanceTransactionDTO) for row in result.scalars().all()
	]


async def create(
	session: AsyncSession,
	dto: BalanceTransactionCreateDTO,
) -> BalanceTransactionDTO:
